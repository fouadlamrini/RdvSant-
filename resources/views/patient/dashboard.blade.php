    ...
    <header class="bg-gradient-to-r from-blue-600 to-blue-800 text-white p-4 shadow-md">
        <div class="container mx-auto flex justify-between items-center">
            ...
            <div class="flex items-center space-x-4">
                ...
                <a href="{{ route('logout') }}"
                    class="flex items-center text-white hover:text-blue-200 transition-colors">
    ...
    <nav class="bg-white shadow-sm">
        <div class="container mx-auto px-4">
            <div class="flex justify-center space-x-8 py-3">
                ...
                <a href="{{ route('patient.mesReservations') }}" class="text-gray-600 hover:text-blue-600 transition-colors pb-2 px-1">Mes Réservations</a>
            </div>
        </div>
    </nav>
    ...
    <main class="container mx-auto px-4 py-8">
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            ...
            <div class="bg-gray-50 px-6 py-8">
                <div class="max-w-3xl mx-auto mb-6 flex flex-wrap gap-4 justify-center">
                    <a href="{{ route('patient.mesRendezVous') }}"
                        class="w-full sm:w-auto bg-blue-600 hover:bg-blue-700 text-white font-medium px-6 py-3 rounded-lg shadow-md flex items-center gap-2 justify-center">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M8 7V3m8 4V3m-9 8h10m-4 12v-6m0 0a9 9 0 10-9-9 9 9 0 009 9z" />
                        </svg>
                        Mes rendez-vous ({{ $totalAppointments }})
                    </a>
                </div>
                ...
                <form action="{{ route('doctors.search') }}" method="GET"
                    class="max-w-3xl mx-auto grid md:grid-cols-3 gap-6">
                    <div>
                    ...
                    <div class="flex items-end">
                        <button type="submit"
                            class="w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-3 px-6 rounded-lg transition duration-300 shadow-md">
                            Rechercher
                        </button>
                    </div>
                </form>
            ...
            <div class="p-6">
                ...
                <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach ($doctors as $doctor)
                        <div
                            class="bg-white border border-gray-200 rounded-xl p-5 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
                            <div class="flex flex-col items-center text-center">
                                <img src="{{ $doctor->image ? asset('storage/' . $doctor->image) : 'https://previews.123rf.com/images/belopoppa/belopoppa1809/belopoppa180900002/109693900-image-d-espace-r%C3%A9serv%C3%A9-de-profil-silhouette-grise-pas-de-photo-d-une-personne-sur-l-avatar-la.jpg' }}"
                                    alt="Dr. {{ $doctor->first_name }} {{ $doctor->last_name }}"
                                    class="w-20 h-20 rounded-full object-cover border-4 border-blue-100 mb-4">
                                <h4 class="font-bold text-lg text-gray-800">Dr. {{ $doctor->first_name }}
                                    {{ $doctor->last_name }}</h4>
                                ...
                                data.forEach(slot => {
                                    if (slot.status === 'available') {
                                        ...
                                        if (slotDate !== currentDate) {
                                            ...
                                            const dateHeader = document.createElement(
                                                'h4');
                                            dateHeader.className =
                                                'font-semibold text-gray-700 mt-4 mb-2 text-lg';
                                            dateHeader.textContent = slotDate.charAt(0)
                                                .toUpperCase() + slotDate.slice(1);
                                            availabilityContent.appendChild(dateHeader);
                                        }

                                        // Create time slot button
                                        const slotElement = document.createElement(
                                            'button');
                                        slotElement.className =
                                            'w-full text-left p-3 border border-gray-200 rounded-lg hover:bg-blue-600 hover:text-white transition-colors mb-2';
